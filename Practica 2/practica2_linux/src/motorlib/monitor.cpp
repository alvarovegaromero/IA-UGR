...
void MonitorJuego::inicializar(int pos_fila, int pos_col, int bruj, int seed) {
  ...
  if ( (nivel==0) or (nivel == 1) or (nivel == 2) or (nivel == 3)) {
    ...
    if (nivel!=3){
      ...
      nueva_entidad(new Entidad(jugador, jugador_,static_cast<Orientacion>(bruj), pos_fila, pos_col, new Jugador3D(""), new ComportamientoJugador(mAux), objetivosActivos.size()/2, objetivosActivos, 3000));
    }
  }
  else {
    // Esto coloca al jugador en el mapa
  ...
  if (nivel == 4) {
    for (int i=0; i<naldeanos; i++) {
      ...
      while( (celdaRand == 'P') or (celdaRand == 'M') or (getMapa()->entidadEnCelda(pos_fila, pos_col) != '_') );
      nueva_entidad(new Entidad(npc, aldeano, norte, pos_fila, pos_col, new Aldeano3D(""), new ComportamientoAldeano(), 0, objetivosActivos, 3000));
    }
  }
