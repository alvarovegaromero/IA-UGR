...
int Environment::RevisarTablero() const{
  ...
  int ganador=0;
  vector<bool>conseguido;
  conseguido.push_back(false);
  conseguido.push_back(false);

  for (int i=0; i<7; i++)
    for (int j=0; j<7; j++){
        if (maze_[i][j]!=0){
          bool resultado=EnLinea(i,j,ganador);
          if (resultado){
            conseguido[ganador-1]=true;
            cuatro_en_raya = true;
          }
        }
    }
    ...
    else { // alguno de los dos o los dos hicieron 4 en cuatro_en_raya
      if (conseguido[0] != conseguido[1]){
        if (conseguido[0]) ganador = 1; else ganador = 2;
      }
      else { // los dos hicieron 4 en cuatro_en_raya, gana el que hizo la ultima accion
        int jugo = (n_movimiento-1)/2;
        if (jugo%2 == 0){ // jugó el jugador 1 y él gana
          ganador = 1;
        }
        else {
          ganador = 2;
  ...
  // if (!cuatro_en_raya and n_jugada > 20){
  //    ganador = 2;
  // }
